...
const createPotProgress = async (potProgress: AddPotProgress) => {
    try {
        ...
        await db.query('INSERT INTO pot_progress (id, saving_goal, amount_saved_per_interval) VALUES ($1, $2, $3)', [potProgressId, potProgress.savingGoal, potProgress.amountSavedPerInterval]);
...
const getPotProgressById = async (potProgressId: string) => {
    try {
        const potProgress = await db.query('SELECT * FROM pot_progress WHERE id = $1', [potProgressId]);
