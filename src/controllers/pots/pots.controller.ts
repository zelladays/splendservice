...
import { parseAddPot, parseUpdatePot, potService } from "../../services";
...
const updateExistingPot = async (req: Request, res: Response) => {
  try {
    if (parseUpdatePot(req.body).success === false) {
      res.status(400).send({
        errorMessage:
          "Invalid request. Please check the request body and try again.",
      });
    ...
    const result = await potService.updatePot(req.body);
